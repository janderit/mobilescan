...
v0.1 (capture + share), v0.2 (crop/rotate) and v0.3 (brightness/contrast) are implemented; next is v0.4 (UI polish). The repository contains the product spec (`README.md`),
...
- **Brightness/contrast** uses one slider with a segmented toggle (brightness 0.5..1.5, contrast 0.5..2.0, neutral 1.0, tick at neutral). The view shows the frame region only. The preview is a CSS filter `brightness(b) contrast(c)` on the display canvas (no pixel work while dragging); on confirm the same filter is baked into the whole captured image via the 2D context `filter` property, with a per-pixel lookup-table loop (`src/tone.ts`) as fallback for contexts without `filter`. Neutral values leave the image untouched; values reset to neutral on each visit and edits accumulate in the image.
